...
void handler_ds18b20(bc_ds18b20_t *self, uint64_t device_address, bc_ds18b20_event_t event, void *event_param)
{
    if (event == bc_ds18b20_EVENT_UPDATE)
    {
        ...
        if (bc_ds18b20_get_temperature_celsius(self, device_address, &ds_value))
        {
           if (ds_value < 80.f)
           {
...
void gps_module_event_handler(bc_module_gps_event_t event, void *event_param)
{
    ...
    else if (event == BC_MODULE_GPS_EVENT_UPDATE)
    {
        ...
        {
            bc_log_debug("Date: %04d-%02d-%02d",time.year, time.month, time.day);
            bc_log_debug("Time: %02d:%02d:%02d",time.hours, time.minutes, time.seconds);
        }
        ...
        {
            bc_log_debug("Lat: %03.5f", position.latitude);
            bc_log_debug("Lon: %03.5f", position.longitude);
            lat = position.latitude * 100000;
            lon = position.longitude * 100000;
        ...
        {
            bc_log_debug("Altitude: %.1f %c", altitude.altitude, tolower(altitude.units));
            alt = altitude.altitude * 10;
        }
        ...
        {
            bc_log_debug("fix: %d", quality.fix_quality);
            bc_log_debug("sat: %d", quality.satellites_tracked);
        }
